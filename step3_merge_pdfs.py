...
OUTPUT_FILENAME = "Output/ClaudeCodeDocs.pdf"
PDFS_DIR = "temp/pdfs"
...
def build_precise_toc(tree, page_start_map):
    ...
    def process_nodes(nodes, parent_title=None, level_offset=0):
        for node in nodes:
            has_children = "children" in node and node["children"]
            href = node.get("href", "")
            level = node.get("level", 1) + level_offset
            title = node["title"]

            # Skip intermediate groups that share the same name as their parent
            # (e.g., Resources > Resources > Legal and compliance, or Glossary > Glossary,
            #  or Getting started > Getting started)
            is_redundant_group = (
                parent_title is not None
                and title == parent_title
            )

            if is_redundant_group:
                # Skip this redundant level, but still process its children (if any)
                # Children move up one level (level_offset - 1 to compensate for skipped parent)
                if has_children:
                    process_nodes(node["children"], parent_title, level_offset - 1)
                # If it's a leaf, just skip it entirely
            elif has_children:
                ...
                if first_child_page:
                    raw_toc.append([level, title, first_child_page])
                # Then process children
                process_nodes(node["children"], title, level_offset)
            else:
                # Leaf page: point to its starting page
                if href in page_start_map:
                    raw_toc.append([level, title, page_start_map[href]])
...
def main():
    ...
    # Cover
    cover_pdf = Path(COVER_DIR) / "Cover_Claude_Code.pdf"
    if cover_pdf.exists():
